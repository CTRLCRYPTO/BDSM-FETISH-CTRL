...
// Copyright (c) 2018-2019 The BDSM-FETISH(CTRL) developers
...
const string strMessageMagic = "BDSM-FETISH Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BDSM-FETISH", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "BDSM-FETISH BEGINS";
        CTransaction txNew;
        if (!fTestNet)
        {
            txNew.nTime = 1443884254;
        }
        else
        {
            txNew.nTime    = 1443883111; // Without separate txNew.nTime near block.nTime, CheckBlock fails on coinbase timestamp for TestNet created on 14042015
        }
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        // debug print
        assert(block.hashMerkleRoot == uint256("0xcae3bfe073873aa6e6e932aee029216f85c9edad92a41360fb8f611b67960655"));
        block.print();
